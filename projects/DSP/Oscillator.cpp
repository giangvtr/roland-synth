...
float Oscillator::dpwSquare()
{
    // DPW square = saw(phase) - saw(phase + 0.5)
 
    //saw 1
    float b1 = 2.f * phaseState - 1.f;
    b1 *= b1;
 
    float out1 = b1 - differentiatorState;
    differentiatorState = b1;
 
    //saw 2
    float phase2 = std::fmod(phaseState + 0.5f, 1.f);
    float b2 = 2.f * phase2 - 1.f;
    b2 *= b2;
 
    float out2 = b2 - differentiatorState2;
    differentiatorState2 = b2;
 
    // subtract and scale - the 0.5 factor corrects the amplitude
    // so output stays in [-1, +1]
    return (out1 - out2) * differentiatorCoeff * 0.5f;
}
